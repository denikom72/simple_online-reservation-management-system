...
// EXAMPLE FOR PREPARE STMT FROM ....
SET @TST = 
INSERT INTO reservations ( `cont_id`, `pers_quant`,`reserv_at`, `reserv_until`)
SELECT c.cont_id, ? AS per,  ? AS st,  ?  AS nd FROM containers AS c
WHERE c.cont_id NOT IN (
         SELECT r.cont_id FROM reservations AS r WHERE
         c.capacity > c.per
         AND (
                 UNIX_TIMESTAMP(c.st) > r.reserv_at AND UNIX_TIMESTAMP(c.st) < r.reserv_until
                                OR
                 UNIX_TIMESTAMP(c.nd) > r.reserv_at AND UNIX_TIMESTAMP(c.nd) < r.reserv_until
                                OR
                 UNIX_TIMESTAMP(c.st) < r.reserv_at AND UNIX_TIMESTAMP(c.nd) > r.reserv_at
        )
) LIMIT 0,1;

SET @a = 2;
SET @b = '2017-12-06 00:22:38';
SET @c = '2017-12-06 03:22:38';

PREPARE STMT FROM @TST;
...
CREATE TABLE `reservations` (
  ...
  UNIQUE KEY `reserv_id_2` (`reserv_id`,`reserv_until`),
  UNIQUE KEY `reserv_id_3` (`reserv_id`),
  KEY `FK_reservations` (`cont_id`),
  CONSTRAINT `FK_reservations` FOREIGN KEY (`cont_id`) REFERENCES `containers` (`cont_id`) ON DELETE CASCADE ON UPDATE CASCADE
